...
const HEADER_BG_CLASSES = {
  "blueprint-blue": "bg-blueprint-blue",
  "blueprint-navyblue": "bg-blueprint-navyblue",
  "blueprint-accent-purple": "bg-blueprint-accent-purple",
  "blueprint-accent-mediumBlue": "bg-blueprint-accent-mediumBlue",
} as const;

export type EvaluationCardProps = {
  title: string;
  body: string;
  /** Tailwind-safe header fill; maps to full `bg-*` classes (JIT cannot see dynamic `bg-${x}`). */
  colour?: keyof typeof HEADER_BG_CLASSES;
};
...
const EvaluationCard = ({
  ...
  colour = "blueprint-blue",
    ...
    const clipId = useId();
    const headerBg = HEADER_BG_CLASSES[colour] ?? HEADER_BG_CLASSES["blueprint-blue"];
    ...
    return (
      <>
        ...
        <div className="inline-flex tablet:hidden desktop:inline-flex max-w-[630px] min-w-[328px] h-[341px] pb-[30px] pt-[42px] px-3 relative bg-white rounded-xl overflow-hidden flex-col items-start">
          ...
          <div
            className={`absolute left-0 right-0 top-0 h-[102px] min-w-[302px] max-wfull max-w-[605px] ${headerBg} rounded-xl m-3`}
            style={{ clipPath: `url(#${clipId})` }}
            aria-hidden
          />
        ...
        <div className="hidden max-w-[902px] min-w-[630px] min-h-[145px] pl-3 pr-12 py-3 bg-white rounded-xl overflow-hidden tablet:inline-flex desktop:hidden flex-col">
          <div className="self-stretch inline-flex justify-start items-center h-[121px] gap-2 my-2">
            <div className="relative w-64 h-32">
              <div
                className={`absolute ${headerBg} rounded-l-xl inset-y-0 left-0 w-8 min-h-[121px]`}
                aria-hidden
              />
              <div
                className={`relative min-h-[121px] ${headerBg} text-white flex items-center justify-center h-full rounded-xl pl-7 pr-[31px] pt-6 pb-9 py-3 origin-bottom-left skew-x-[-8deg]`}
              >
